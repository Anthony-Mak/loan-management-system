...
<body>
  ...
  <!--JavaScript code that handles login and authentication-->
  <script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    loginForm.addEventListener('submit', (e) => {
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          // Always redirect regardless of password_change_required flag
    ...
    function debugTokens() {
      const metaToken = document.querySelector('meta[name="csrf-token"]')?.content;
      console.log('Current Meta CSRF Token:', metaToken);
      
      const cookieToken = document.cookie.split('; ')
        .find(row => row.startsWith('XSRF-TOKEN='))
        ?.split('=')[1];
      console.log('Current Cookie CSRF Token:', cookieToken ? decodeURIComponent(cookieToken) : 'Not found');
      
      const authToken = localStorage.getItem('auth_token');
      console.log('Current Auth Token:', authToken ? authToken.substring(0, 10) + '...' : 'Not found');
    }
  });
  </script>
</body>
